...
para(tf, "MERIDIAN FAMILY OFFICE COPILOT · v6", 11.5, GOLD, bold=True, first=True, ls=1.6)
...
# ---- Stage 3 · two outputs (stacked) -------------------------------------- #
x3 = x2 + w2 + 0.5
w3 = 12.53 - x3
oh = 1.62
gap = 0.26
top = SPINE - (oh * 2 + gap) / 2

# Output A — monitoring watchlist
b = box(x3, top, w3, oh, fill=GOLD_TINT, line=GOLD, line_w=1.5, radius=0.08)
tf = b.text_frame
para(tf, "\U0001F4E1  Monitoring watchlist", 14, INK, bold=True, first=True)
para(tf, "Level-based triggers to watch the book against.", 11.5, SOFT, before=5, spacing=1.1)
p = tf.add_paragraph(); p.space_before = Pt(6)
for seg, col, bold in [("S&P 500 · ", INK, True), ("−15/−20%", GOLD_DK, True),
                       ("     Gold · ", INK, True), ("< $4,000", GOLD_DK, True)]:
    r = p.add_run(); r.text = seg; r.font.size = Pt(11); r.font.name = MONO
    r.font.color.rgb = col; r.font.bold = bold
lab = txt(x3 + w3 - 2.5, top + 0.12, 2.35, 0.32, anchor=MSO_ANCHOR.TOP)
para(lab, "LIVES IN THE COPILOT", 9.5, GOLD_DK, bold=True, first=True, align=PP_ALIGN.RIGHT, ls=0.6)

# Output B — analyst guidance
by = top + oh + gap
b = box(x3, by, w3, oh, fill=SLATE_TINT, line=LINE_STRONG, line_w=1.25, radius=0.08)
tf = b.text_frame
para(tf, "\U0001F9ED  Analyst guidance", 14, INK, bold=True, first=True)
para(tf, "Intent that shapes the written advice — never the figures.", 11.5, SOFT,
     before=5, spacing=1.1)
para(tf, "Execution style · selection criteria · open questions", 11, SOFT, font=MONO, before=6)
lab = txt(x3 + w3 - 3.3, by + 0.12, 3.15, 0.32, anchor=MSO_ANCHOR.TOP)
para(lab, "PROPOSAL DECK · CIO COMMENTARY", 9.5, FAINT, bold=True, first=True,
     align=PP_ALIGN.RIGHT, ls=0.5)

# small branching chevron into the gap between the two output cards
